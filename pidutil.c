...
pidfile_write(struct pidfh *pfh)
{
	 ...
	 * if pidfile was closed and descriptor reused.
	 */
	errno = pidfile_verify(pfh);
	if (errno != 0) {
		/*
		 * Don't close descriptor, because we are not sure if it's
		 * ours.
		 */
		return (-1);
	}
	...
	/*
	 * Truncate PID file, so multiple calls of pidfile_write() are
	 * allowed.
	 */
	if (ftruncate(fd, 0) == -1) {
		error = errno;
